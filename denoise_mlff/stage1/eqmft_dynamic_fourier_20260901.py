"""Dynamic-Fourier mean-field EqMFT Stage-1 pilot without augmentation.
...
The logical optimizer batch is conservatively reduced to 10,000 atoms for this
pilot.  Its processed-atom scheduler milestones match the 25,600-atom M=1
control, but its optimizer-update count and gradient noise do not; it is not an
architecture-only A/B.  Physical subdivision caps are unprofiled starting
values that require a representative CUDA forward/backward before a serious
launch.  Exact O(3) covariance and cell-repetition invariance make random
rotation and supercell multiplicity augmentation unnecessary for this M=1
pilot.
"""
...
TARGET_TRAIN_BATCH_ATOMS = 10_000
MAX_ATOMS = 100
MAX_SUPERCELL_MULTIPLICITY = 1
# Unprofiled physical caps. The 5.595-GiB reference starts below the measured
# packed-v2 M=1 workload because the dynamic path has denser reciprocal modes
# and repeats live site-to-mode contractions at six depths.
REFERENCE_GPU_MEMORY_GIB = 5.595
REFERENCE_MICROBATCH_ATOMS = 400
MICROBATCH_ATOM_GRANULARITY = 100
REFERENCE_MICROBATCH_EDGES = 25_000
        ...
        return dict(
            ...
            lr_scheduler=dict(
                ...
                params=dict(
                    name="exp_warmup",
                    params=dict(
                        # Match the M=1 control's processed-atom schedule after
                        # reducing 25,600 atoms/update to 10,000.
                        warmup_steps=960,
                        decay_steps=19_200,
                        min_lr_ratio=0.01,
                    ),
