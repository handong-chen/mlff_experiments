"""Field-256 block-tied sparse dynamic MF v3 Stage-1 experiment.

Sibling of eqmft_fourier_v3_sparse_tied4_site64_20260906.  It retains 64-wide
site tokens and local-refresh inputs, while all field-token channels are 256.
The 24 physical layers, four-layer tying, writer hidden width, and local/Fourier
refreshes at layers 0, 4, 8, 12, 16, and 20 are unchanged.

Physical atom and edge caps are 7/8 of the site64/field128 sibling.  This
accounts for the matched FP16 peak-memory increase measured when field width
doubles, while retaining the logical 25,600-atom batch.  These are conservative
starting caps, not a capacity result for every graph packing or GPU tier.
"""
...
# 7/8 of the published site64/field128 caps.  The measured field128 -> field256
# FP16 peak allocation change was +12.55 percent on a matched real batch.
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v3-sparse-tied4-site64-field256] "
            ...
            model=dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        dynamic_reciprocal_reference_atomic_volume_angstrom3=15.0,
                        dynamic_reciprocal_gate_init=0.1,
                        dynamic_reciprocal_gate_max=1.0,
                        architecture_version="eqmft_denoiser_meanfield_dynamic_fourier_v3",
