"""Stage-1 deep dense-Transformer pretrain with a 12.8k-atom logical batch.

Relative to the d384/L12 sibling, this model halves the embedding width and
doubles the depth. Three heads retain the 64-channel head dimension, while the
larger logical atom budget reduces optimizer-step frequency. Lower physical
caps account for the deeper Full-AttnRes activation history under AMP.
"""
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        return dict(
            model=dict(
                name="remote_import.mlff.model.build_transformer_denoiser",
                params=dict(
                    config=dict(
                        n_elements=119,
                        d_model=192,
