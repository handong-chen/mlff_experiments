"""Wider-site block-tied sparse dynamic MF v3 Stage-1 experiment.

Sibling of eqmft_fourier_v3_sparse_tied4_20260906. Site channels and atom
embedding width are 64, so local refresh consumes 64 channels without a
32-channel projection bottleneck. Field width 128, writer hidden width 256,
24 physical layers, four-layer weight sharing, and local/Fourier refreshes
at zero-based layers 0, 4, 8, 12, 16, and 20 are unchanged.

Physical atom and edge caps are 70 percent of the width-32 sibling's limits,
budgeting for the counted 1.37x FP32 saved-forward-tensor storage increase.
The logical 25,600-atom batch, optimizer, data split, and validation batch size
are unchanged. These are starting caps, not measured OOM-free capacities.
"""
...
# Reduce both physical caps to 70 percent of the width-32 sibling.
# 0.70 * 1.37 is about 0.96 of its counted activation storage; this is an
# estimate, not a measured peak-AMP-memory or OOM-free capacity guarantee.
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v3-sparse-tied4-site64] "
