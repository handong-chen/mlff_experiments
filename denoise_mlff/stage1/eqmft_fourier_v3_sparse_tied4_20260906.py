"""Block-tied sparse dynamic MF v3 Stage-1 experiment.

Each four-layer block shares one mean-field update module. AttnRes queries
remain depth-specific; all 24 updates and both sparse refresh schedules remain.
The only model-config change is layer_weight_sharing_block_size=4.

Derived from eqmft_fourier_v2_20260901 with its six Fourier refreshes retained.
Local geometry is also refreshed at zero-based layers 0, 4, 8, 12, 16, and 20,
using the learned site scalars in the existing width256 geometry MLP. Local
refresh occurs before the selected mean-field layer; Fourier refresh remains
after that layer. No site-resolved fields or additional transport branch.

All sparse-v2 widths, cutoff, optimizer, data split, logical atom budget, and
physical subdivision caps are retained. The inherited caps are starting limits,
not measured capacities for this sparse v3 configuration.
"""
...
# Inherited sparse-v2 starting limits, not measured sparse-v3 capacities.
# Profile the added local refresh activations before a serious launch.
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v3-sparse-tied4] "
