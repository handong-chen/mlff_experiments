"""Fresh width-128 frozen-mat2vec Stage 1 with Muon and auxiliary AdamW.

Matches eqmft_fourier_v3_sparse_site128_mat2vec_frozen_20260926 except for the
optimizer. Muon updates trunk channel matrices independently per packed token;
AdamW updates biases, gains, routing queries and the displacement readout.
Uses PyTorch's match_rms_adamw scaling with the existing learning rate and
weight decay. Frozen mat2vec vectors remain outside both parameter groups.
Physical GPU-memory caps are inherited estimates, not measured Muon capacity.
"""
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        return dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        architecture_version=(
                            "eqmft_denoiser_meanfield_dynamic_fourier_v3_mat2vec"
                        ),
            ...
            optimizer=dict(
                name="remote_import.mlff.pipeline.muon.build_eqmft_muon_optimizer",
                params=dict(
                    lr=1.0e-3,
                    weight_decay=0.01,
                    momentum=0.95,
