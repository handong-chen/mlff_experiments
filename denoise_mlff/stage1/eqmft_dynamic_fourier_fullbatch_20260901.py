"""Full-batch dynamic-Fourier mean-field EqMFT Stage-1 without augmentation.

The fixed local and reciprocal environments match the packed-v2 mean-field
control.  A shared low-rank Fourier residual additionally routes the evolving
site state after layers 0, 4, 8, 12, 16, and 20 using dynamic channels
(16, 8, 4) and every reciprocal vector with |G| < 1.6 inverse Angstrom.
Reciprocal geometry and radial weights are prepared once per physical forward.

The logical optimizer batch remains 25,600 atoms, matching the M=1 control.
Only physical subdivision caps vary with GPU memory.  The caps are based on a
real graph-mmap CUDA forward/backward profile of the vectorized Fourier route;
the 80-GiB atom and edge caps admit the profiled first 25,598-atom batch as one
physical microbatch.  Edge-heavier batches may subdivide without changing the
logical update, and a first-batch 80-GiB CUDA confirmation remains required.
Exact O(3) covariance and cell-repetition invariance make random rotation and
supercell multiplicity augmentation unnecessary.
"""
...
# The under-12-GiB cap was exercised at 1,777 atoms / 63,418 edges with
# 4.13 GiB peak allocated on a 5.595-GiB RTX 2060.  Peak memory scaled nearly
# linearly from 298 through 1,777 atoms.  On one real 25,598-atom batch, mode
# bucketing reduced phase storage from 287.25M globally padded values to 8.16M.
# The larger tiers retain headroom for edge and reciprocal-mode variation.
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-dynamic-fourier] "
                ...
                params=dict(
                    config=dict(
                        ...
                        architecture_version=(
                            "eqmft_denoiser_meanfield_dynamic_fourier_v1"
                        ),
