"""Volume-measured layerwise dynamic-Fourier EqMFT Stage-1 pilot.
...
learned gate after every transformer layer (layers 0 through 23).  Gates start
at 0.1 and are bounded by 1.0.  The 15 Angstrom^3 reference is the rounded
median atomic volume (15.1488 Angstrom^3) of the selected Stage-1 training
split.
...
subdivision caps vary with GPU memory.  Updating Fourier fields at every layer
is intentionally more expensive than the six-refresh control and requires a
representative CUDA profile before a serious launch.  The v1 caps are retained
as starting limits; first-batch CUDA memory and optimizer-step finiteness remain
required launch checks for this checkpoint-incompatible fresh architecture.
"""
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v2-layerwise] "
            ...
            model=dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        # Refresh the dynamic Fourier field after every one of
                        # the 24 layers.
