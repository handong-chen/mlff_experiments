"""Stage-1 pure EqMFT with separate local and reciprocal environments.
...
# Provider reference for an 8-GiB-class card.  Earlier real M=1 and forced-M=2
# smokes exercised 600 atoms / 25k edges on a physical 5.595-GiB RTX 2060 and
# left more than half of device memory free.  The doubled caps below are the
# explicit next-run estimate, not a measured worst-case memory bound.
REFERENCE_GPU_MEMORY_GIB = 7.656
REFERENCE_MICROBATCH_ATOMS = 1_200
MICROBATCH_ATOM_GRANULARITY = 100
REFERENCE_MICROBATCH_EDGES = 50_000
