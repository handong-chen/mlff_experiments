"""Site-128 block-tied sparse dynamic MF v3 Stage-1 experiment.

Sibling of eqmft_fourier_v3_sparse_tied4_site64_20260906. Site channels and
atom embedding width are 128, so local refresh consumes 128 channels without
a projection bottleneck. Field width 128 is unchanged. Radial and invariant
writer hidden widths are 512 and position-readout hidden width is 256, preserving
the site-to-MLP width scaling of the site64 sibling. 24 physical layers,
four-layer weight sharing, and local/Fourier refreshes at zero-based layers 0,
4, 8, 12, 16, and 20 are unchanged.

Physical atom and edge caps are approximately 5/9 of the site64 limits.
Matched CUDA FP16 forward/backward/AdamW probes measure 3.062 GiB at 503 atoms
and 4.489 GiB at 755 atoms; the under-12-GiB cap of 700 atoms estimates
4.18 GiB on the 5.595-GiB profiling GPU. The logical 25,600-atom batch,
optimizer, data split, and validation batch size are unchanged. These are
measured-scaling starting caps, not certified OOM-free capacities.
"""
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v3-sparse-tied4-site128] "
