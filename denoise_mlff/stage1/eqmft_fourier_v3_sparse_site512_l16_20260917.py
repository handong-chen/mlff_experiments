"""Untied site-512, 16-layer sparse dynamic MF v3 Stage-1 experiment.

Sibling of eqmft_fourier_v3_sparse_site256_20260917. Site channels and
atom embedding/local-refresh projection width double to 512; radial and
invariant MLP widths double to 2048 and position readout width to 1024.
Field channels remain 128. Depth decreases from 24 to 16 independent
layers, with local and Fourier refresh at layers 0, 4, 8, and 12.

Physical atom/edge caps start at approximately two-thirds of the updated
site256 caps. Width-linear, layer-dependent activations scale by roughly
(512 / 256) * (16 / 24) = 4/3 at a matched physical batch, but parameter
and optimizer storage grow faster than activations. These budgets are
provisional estimates, not measured or certified OOM-free capacities;
fixed storage can dominate on small GPUs. Logical batch targets, data,
and optimization are unchanged.
"""
...
# Approximately two-thirds of the updated site256 physical budgets.
# Preserve the logical batch target via physical microbatch accumulation.
# Smaller-tier budgets do not certify that the wider model's fixed state fits.
...
class ConfigProvider:
    def __call__(self, *args, **kwargs):
        ...
        print(
            "[denoise-mlff][elastic-eqmft-fourier-v3-sparse-site512-l16] "
            ...
            model=dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        position_readout_hidden_dim=1024,
