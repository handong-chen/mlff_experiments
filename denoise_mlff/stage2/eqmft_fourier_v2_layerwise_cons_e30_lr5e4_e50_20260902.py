"""Conservative layerwise Fourier-v2 Stage-2 finetune from Stage-1 epoch 30.

The Stage-1 checkpoint transfers the complete dynamic-Fourier-v2 trunk with a
refresh after every layer. Stage 2 adds a fresh invariant energy readout;
forces and stress are derivatives of the summed physical energy. The 10,000
atom logical batch leaves headroom for the layerwise route's larger conservative
training graph while remaining one physical microbatch on an 80GB A100. The
learning rate warms to 5e-4 over approximately half an epoch and exponentially
decays to a 5e-5 floor at epoch 50.
"""
...
STAGE1_CHECKPOINT = os.path.join(
    ...
    "denoise_mlff",
    "stage1",
    "eqmft_fourier_v2_layerwise_20260902",
    "mlff=27520eb;workshop=9afaf97",
    "epoch_000030.pt",
)
        ...
        return dict(
            model=dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        dynamic_reciprocal_refresh_layers=(
                            0, 1, 2, 3, 4, 5, 6, 7,
            ...
            optimizer=dict(
                ...
                params=dict(
                    ...
                    params=dict(lr=5.0e-4, weight_decay=1.0e-3),
                ),
            ),
            lr_scheduler=dict(
                ...
                params=dict(
                    name="exp_warmup",
                    params=dict(
                        # Preserve the established half-epoch warmup. The decay
                        # constant places the 0.1 floor at approximately epoch 50
                        # for the 10,000-atom logical batch.
                        warmup_steps=1_967,
                        decay_steps=84_625,
                        min_lr_ratio=0.1,
                    ),
