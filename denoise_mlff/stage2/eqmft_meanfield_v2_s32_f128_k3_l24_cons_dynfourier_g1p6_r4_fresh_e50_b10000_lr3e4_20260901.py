"""Fresh sparse dynamic-Fourier mean-field EqMFT Stage-2 variant.

The existing local and reciprocal environments remain fixed within a forward,
and the three common field tokens retain their every-layer routing.  A shared
low-rank reciprocal residual additionally transports the evolving site state
after layers 0, 4, 8, 12, 16, and 20.  Reciprocal geometry and phases are
constructed once per forward and reused by all six refreshes.  This distinct
architecture starts without a Stage-1 or Stage-2 checkpoint initializer.
"""
        ...
        return dict(
            ...
            lr_scheduler=dict(
                ...
                params=dict(
                    name="exp_warmup",
                    params=dict(
                        # Preserve the baseline's processed-atom warmup and
                        # decay after reducing 12,800 atoms/step to 10,000.
                        warmup_steps=1_967,
                        decay_steps=39_365,
                        min_lr_ratio=0.01,
                    ),
