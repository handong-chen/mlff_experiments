"""Conservative site-128 sparse Fourier-v3 Stage-2 finetune from epoch 20.

Transfers the complete tied-four Stage-1 trunk with 128-wide site/local and
field features, 512-wide radial/invariant MLPs, and 24 physical layers.
Local and Fourier refreshes remain at layers 0, 4, 8, 12, 16, and 20.
A fresh 256-wide invariant energy readout supplies physical energies and
their conservative force/stress derivatives.

Physical atom caps are half the corrected site64 Stage-2 caps, rounded down
to multiples of 16. This budgets for an estimated 2x memory increase from
doubling site and MLP widths; the matched Stage-1 FP16 probe measured 1.78x,
but Stage-2 FP32 conservative double-backward memory has not been measured
for site128. These are conservative starting estimates, not measured capacities.
Validation batches are also halved while retaining the 128-sample cap.
The logical 12,000-atom optimizer batch, loss, 50-epoch duration, and schedule
follow eqmft_fourier_v3_sparse_tied4_site64_cons_e20_20260907.
"""
...
STAGE1_CHECKPOINT = os.path.join(
    ...
    "denoise_mlff",
    "stage1",
    "eqmft_fourier_v3_sparse_tied4_site128_20260908",
            ...
            model=dict(
                ...
                params=dict(
                    config=dict(
                        n_elements=119,
                        n_layers=24,
                        layer_weight_sharing_block_size=4,
                        n_common_tokens=3,
                        l_max=2,
