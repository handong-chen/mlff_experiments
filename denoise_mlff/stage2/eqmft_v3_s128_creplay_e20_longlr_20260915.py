"""Compiled-replay sibling with the original sparse-v3 learning-rate schedule.

Preserves the model, epoch-20 initializer, optimizer, data, replay settings and
batch budgets of eqmft_v3_s128_creplay_e20_20260915; that provider is unchanged.
Copies exp_warmup step counts and minimum LR ratio exactly from
eqmft_fourier_v3_sparse_cons_e20_20260907, without batch-size rescaling.
    ...
    def __call__(self, *args, **kwargs):
        ...
        return dict(
            ...
            trainer=dict(
                ...
                eqmft_history=True,
                eqmft_field_atom_capacity=None,
                eqmft_layer_replay=dict(
                    atom_capacity=batch_plan.max_train_microbatch_atoms,
                    # At most one nonempty structure per atom; replay adds
                    # its own extra dummy graph beyond this real-graph cap.
                    graph_capacity=batch_plan.max_train_microbatch_atoms,
                    sites_per_graph=1 << (MAX_ATOMS - 1).bit_length(),
                    bucket_shapes=True,
