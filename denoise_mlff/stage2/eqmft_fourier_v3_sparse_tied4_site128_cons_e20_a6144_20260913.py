"""Higher-physical-budget eager sibling of the tied-four site128 Stage-2 run.

Baseline: eqmft_fourier_v3_sparse_tied4_site128_cons_e20_20260908.
The 80GB physical atom cap increases from 3,696 to 6,144. A full
12,000-atom logical batch then needs two rather than four physical calls.
This is an aggressive planning budget, not a measured A100 capacity guarantee.

Lower-memory physical caps, the logical batch, validation sampling, checkpoint,
architecture, optimizer, and scheduler retain the baseline settings.
Compiled fields and history optimization are explicitly disabled.
"""
...
MAX_ATOMS = 100
# The 80GB cap is 6,144 / 3,696 = 1.662 times the baseline capacity.
# With at most 100 atoms per graph, a closed first microbatch contains at least
# 6,045 atoms; the remaining at most 5,955 fit in a second physical call.
# Actual peak memory depends on graph composition; no OOM-free fit is claimed.
# Lower-memory tiers retain the baseline budgets.
