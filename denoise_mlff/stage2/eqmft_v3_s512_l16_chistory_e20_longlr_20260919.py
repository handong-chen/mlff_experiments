"""Site-512, 16-layer conservative Stage-2 with compiled fields and history.

Standalone sibling of eqmft_v3_s128_chistory_e20_longlr_20260915.
Loads epoch 20 of eqmft_fourier_v3_sparse_site512_l16_20260917
from mlff=0cdb8f0;workshop=9afaf97. Trunk widths, depth, and local/Fourier
refresh schedules match that Stage-1 provider. The fresh energy head scales
with site width. Field channels remain 128 and all layers remain untied.

Preserves the original sparse-v3 long LR schedule in optimizer steps, the
16,000-atom logical batch, conservative loss, and 128-sample validation cap.
Compiled fields and analytical history are enabled; layer replay is not.
Physical train and validation capacities are reduced for the wider model.
Budgets are estimates, not measured GPU peaks or certified OOM-free fits.
Checkpoint existence and loading have not been checked.
"""
...
MAX_ATOMS = 100
# Reference: site128 compiled-history uses an 8,192-atom 80GB cap, estimated
# from roughly 32 GiB allocated at 4,096 atoms, not a measured 8,192-atom peak.
# Width-linear, layer-dependent activation factor versus site128/l24:
# 4 * (16 / 24) = 8/3. Fixed parameter/gradient/AdamW storage grows separately.
# The 80GB cap leaves room for that fixed-state increase and targets
# 6 physical calls for 16,000 atoms: 6 * (2816 - 99) >= 16,000.
# The 99-atom allowance covers greedy packing with graphs of at most 100 atoms.
# Compilation padding capacity equals the actual physical atom cap below.
# Smaller tiers include additional fixed-state headroom; a minimum 100-atom
# cap does not establish that the model fits a small GPU.
...
STAGE1_CHECKPOINT = os.path.join(
    ...
    "denoise_mlff",
    "stage1",
    "eqmft_fourier_v3_sparse_site512_l16_20260917",
    "mlff=0cdb8f0;workshop=9afaf97",
    "epoch_000020.pt",
)
        ...
        return dict(
            model=dict(
                ...
                params=dict(
                    config=dict(
                        ...
                        site_channels=(512, 512, 512),
                        field_channels=(128, 128, 128),
                        qk_channels=(8, 8, 8),
                        n_heads=4,
                        atom_embedding_dim=512,
                        n_rbf=32,
                        radial_hidden_dim=2048,
                        invariant_hidden_dim=2048,
                        attn_res_key_dim=64,
                        ...
                        dynamic_reciprocal_refresh_layers=(0, 4, 8, 12),
                        dynamic_local_refresh_layers=(0, 4, 8, 12),
            ...
            lr_scheduler=dict(
                ...
                params=dict(
                    name="exp_warmup",
                    params=dict(
                        # Match eqmft_fourier_v3_sparse_cons_e20_20260907
                        # exactly in optimizer steps, without batch rescaling.
                        warmup_steps=1_537,
                        decay_steps=30_754,
                        min_lr_ratio=0.01,
                    ),
